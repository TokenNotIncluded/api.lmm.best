...
async function capture(page, name, errors) {
  ...
  report.push({ name, viewport: page.viewportSize(), dimensions, errors: [...errors] })
}

try {
  for (const width of [1440, 834, 390, 320]) {
    const context = await browser.newContext({ viewport: { width, height: 1000 }, locale: 'zh-CN', serviceWorkers: 'block' })
    const errors = []
    ...
    try {
      await page.goto(`${origin}/?ui_review=1`, { waitUntil: 'domcontentloaded' })
      ...
      assert.ok(track && thumb && thumb.x >= track.x && thumb.x + thumb.width <= track.x + track.width + 1)
      ...
      await page.waitForFunction(() => document.activeElement?.getAttribute('data-slot') === 'input-otp-slot')
      ...
      await page.waitForFunction(() => [...document.querySelectorAll('[data-slot=input-otp-slot]')].map(el => el.value).join('') === '123456')
      assert.equal(await page.getByTestId('otp-submit-count').innerText(), '0')
      assert.equal(await form.evaluate(el => new FormData(el).get('otp')), '123456')
      await form.getByRole('button', { name: '验证', exact: true }).click()
      await page.getByTestId('otp-submit-count').filter({ hasText: '1' }).waitFor()
      const dialogTrigger = page.getByRole('button', { name: '打开弹窗', exact: true })
      await dialogTrigger.click()
      const dialog = page.getByRole('dialog', { name: '连接设置' })
      await dialog.getByRole('combobox', { name: '连接模式' }).click()
      await page.getByRole('option', { name: '低延迟连接', exact: true }).click()
      assert.equal(await dialog.isVisible(), true)
      await capture(page, `dialog-${width}`, errors)
      await dialog.getByRole('button', { name: '完成', exact: true }).click()
      await dialog.waitFor({ state: 'hidden' })
      assert.equal(await dialogTrigger.evaluate(el => el === document.activeElement), true)
      const drawerTrigger = page.getByRole('button', { name: '打开抽屉', exact: true })
      await drawerTrigger.click()
      const drawer = page.getByRole('dialog', { name: '筛选记录' })
      await drawer.waitFor()
      await drawer.getByRole('textbox', { name: '搜索', exact: true }).fill('示例记录')
      await drawer.getByRole('textbox', { name: '抽屉备注' }).fill('手势不能吃掉输入')
      const rect = await drawer.boundingBox()
      assert.ok(rect && rect.width > 200 && rect.x >= -1 && rect.x + rect.width <= width + 1)
      await capture(page, `drawer-${width}`, errors)
      await page.keyboard.press('Escape')
      await drawer.waitFor({ state: 'hidden' })
      assert.equal(await drawerTrigger.evaluate(el => el === document.activeElement), true)
      await page.emulateMedia({ reducedMotion: 'reduce' })
      const duration = await toggle.evaluate(el => getComputedStyle(el).transitionDuration)
      assert.equal(duration, '0s')
    } catch (error) {
      await page.screenshot({ path: path.join(output, `failed-${width}.png`), fullPage: true })
...
console.log(`UI foundation: ${report.length} views and interaction contracts passed`)
