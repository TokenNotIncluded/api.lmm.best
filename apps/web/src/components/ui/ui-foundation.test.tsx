...
for (const key of ['window', 'document', 'navigator', 'HTMLElement', 'HTMLInputElement', 'HTMLButtonElement', 'Node', 'Element', 'Event', 'MouseEvent', 'MutationObserver', 'ResizeObserver', 'requestAnimationFrame', 'cancelAnimationFrame'] as const) {
...
async function mount(children: ReactNode) {
  ...
  await act(async () => root.render(<I18nextProvider i18n={i18n}>{children}</I18nextProvider>))
  return { host, cleanup: async () => { await act(async () => root.unmount()); host.remove() } }
}

function OTPExample({ disabled = false, readOnly = false, onChange, onComplete }: { disabled?: boolean; readOnly?: boolean; onChange?: (value: string) => void; onComplete?: () => void }) {
  const [value, setValue] = useState('')
  return <form><label htmlFor='test-otp'>Verification code</label>
    <InputOTP id='test-otp' name='otp' length={6} value={value} disabled={disabled} readOnly={readOnly} aria-invalid='true' aria-describedby='otp-help' autoSubmit={false} onValueChange={next => { setValue(next); onChange?.(next) }} onValueComplete={onComplete}>
      <InputOTPGroup>{Array.from({ length: 6 }, (_, index) => <InputOTPSlot key={index} index={index} />)}</InputOTPGroup>
    </InputOTP><p id='otp-help'>Six digits</p>
  </form>
}
...
test('Luma buttons retain disabled behavior, native refs and anchor rendering', async () => {
  ...
  const view = await mount(<><Button ref={ref} disabled onClick={() => clicks++}>Disabled</Button><Button render={<a href='#target' />}>Link</Button></>)
  try {
...
test('input wrappers preserve native form values, refs, errors and multiline sizing', async () => {
  ...
  const view = await mount(<form><Input ref={ref} name='name' defaultValue='Example' aria-invalid='true' /><Textarea name='notes' defaultValue={'a\nb'} /></form>)
  try {
    ...
    assert.ok(view.host.querySelector('textarea')?.className.includes('max-h-96'))
  } finally { await view.cleanup() }
})
...
test('OTP uses six labelled native inputs and propagates RHF error descriptions', async () => {
  ...
  try {
    const slots = view.host.querySelectorAll<HTMLInputElement>('[data-slot=input-otp-slot]')
...
test('OTP paste normalizes whitespace and completion never submits the form', async () => {
  ...
  let complete = 0
  let submits = 0
  const view = await mount(<OTPExample onChange={next => value = next} onComplete={() => complete++} />)
  try {
    const form = view.host.querySelector('form')
    const first = view.host.querySelector<HTMLInputElement>('[data-slot=input-otp-slot]')
    ...
    assert.equal(view.host.querySelector<HTMLInputElement>('input[name=otp]')?.value, '123456')
  } finally { await view.cleanup() }
})
