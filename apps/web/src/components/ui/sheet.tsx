...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        'fixed inset-0 z-50 transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0 bg-foreground/20 dark:bg-background/70 supports-backdrop-filter:backdrop-blur-sm motion-reduce:transition-none',
        className
      )}
      {...props}
    />
...
function SheetContent({
  ...
  // Keep ALL side-specific geometry in these plain conditional classes.
  // A data-[side=*]: sizing variant in the shared classes has higher CSS
  // specificity and survives tailwind-merge, overriding consumers such as
  // w-full / sm:max-w-5xl and crushing the channel editor's desktop columns.
  return (
    <SheetPortal>
