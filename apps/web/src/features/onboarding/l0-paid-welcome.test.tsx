...
Object.defineProperty(globalThis, 'customElements', {
  configurable: true,
  value: { get() { return undefined }, define() {} },
})
...
const { subscribeToAssistantOpen } = await import('@/features/assistant/assistant-events')
...
after(() => { api.get = originalGet; dom.close() })
...
test('L0 top-up goes straight to wallet without opening an assistant or hiding pending review', async () => {
  ...
  const unsubscribe = subscribeToAssistantOpen((event) => opened.push(event.preset))
  const rootRoute = createRootRoute({ component: Outlet })
  const welcome = createRoute({
    getParentRoute: () => rootRoute,
    path: '/getting-started',
    component: () => <L0Welcome user={user}><p>Pending review</p></L0Welcome>,
  })
  ...
  try {
    ...
    const button = container.querySelector<HTMLButtonElement>('[data-testid="l0-topup-direct"]')
    assert.ok(button)
