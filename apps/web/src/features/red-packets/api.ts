...
export async function getRedPacket(
  ...
  const response = await api.get(`/api/red-packet/${encodeURIComponent(slug)}`, {
...
export async function claimRedPacket(
  ...
  const response = await api.post(
    `/api/red-packet/${encodeURIComponent(slug)}/claim`,
    {}
  )
  return response.data
}
...
export async function getMyRedPacketClaims(
  ...
  const response = await api.get(
    `/api/red-packet/${encodeURIComponent(slug)}/claims/me`,
