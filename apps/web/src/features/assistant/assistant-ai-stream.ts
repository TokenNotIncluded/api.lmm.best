...
export function assistantTimeoutError(code = 'ASSISTANT_REQUEST_TIMEOUT'): AssistantStreamError {
  const message = 'The assistant request timed out. Check completed actions before retrying.'
  return new AssistantStreamError(408, { code, message, retryable: false }, message, false)
}
...
export async function withAssistantDeadline<T>(
  ...
  const abort = () => controller.abort(parent ? assistantAbortReason(parent) : undefined)
  parent?.addEventListener('abort', abort, { once: true })
  const timer = setTimeout(() => controller.abort(assistantTimeoutError()), timeoutMs)
...
export async function consumeAssistantAISDKStream(
  ...
  return withAssistantDeadline(async (signal) => {
    ...
    const resetIdle = () => {
      ...
      idleTimer = setTimeout(() => rejectStopped(assistantTimeoutError('ASSISTANT_STREAM_IDLE_TIMEOUT')),
        options.idleTimeoutMs ?? ASSISTANT_STREAM_IDLE_TIMEOUT_MS)
    }
    const protocolError = (message: string) => new AssistantStreamError(
      502, { message, retryable: false }, message, false
    )
    const dispatch = () => {
      ...
      if (event === 'error') {
        ...
        const message = typeof payload.message === 'string' ? payload.message : 'AI assistant stream failed'
        throw new AssistantStreamError(status, payload, message,
          !activitySeen && (typeof payload.retryable === 'boolean' ? payload.retryable : isRetryableAssistantStatus(status)))
      }
      if (event === 'done') { result = payload; return }
      if (event === 'progress') {
        if ((payload.phase === 'model' || payload.phase === 'tool' || payload.phase === 'answer') &&
            typeof payload.step === 'number' && Number.isInteger(payload.step) && payload.step > 0 && payload.step <= 32) {
          if (payload.phase === 'tool') activitySeen = true
          handlers.onProgress?.({ phase: payload.phase, step: payload.step })
        }
        return
      }
      if ((event === 'delta' || event === 'replace') && typeof payload.content === 'string') {
    ...
    const processLine = (rawLine: string) => {
      ...
      if (eventSize > ASSISTANT_STREAM_EVENT_MAX_CHARS) throw protocolError('Assistant stream event exceeds its size limit')
      if (!line) dispatch()
    ...
    try {
      ...
      while (!result) {
        ...
        if (done) {
          ...
          dispatch()
          if (!result) throw protocolError('Assistant stream ended before completion')
          break
        }
        ...
        if (!result && buffer.length + eventSize > ASSISTANT_STREAM_EVENT_MAX_CHARS) {
      ...
      if (signal.aborted) throw assistantAbortReason(signal)
      if (error instanceof AssistantStreamError || (error instanceof Error && error.name === 'AbortError')) throw error
