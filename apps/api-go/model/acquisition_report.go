...
func GetAcquisitionReport(ctx context.Context, from, to int64) (AcquisitionReport, error) {
	...
	cohort := func() *gorm.DB {
		return db.Table("users AS u").Joins("LEFT JOIN acquisition_accounts AS a ON a.user_id = u.id AND a.created_at >= ?", now-AcquisitionAccountDays*86400).Where("u.created_at >= ? AND u.created_at < ? AND u.role < ? AND u.deleted_at IS NULL", from, to, common.RoleAdminUser)
	}
	...
	events := db.Raw(`
 SELECT t.user_id, CASE WHEN t.complete_time > 0 THEN t.settled_amount_micros ELSE 0 END AS paid,
  0 AS refunded, t.settlement_currency AS currency, t.payment_method AS method, t.payment_provider AS provider,
  t.complete_time AS occurred_at
 FROM top_ups AS t WHERE t.status = ? AND (t.complete_time BETWEEN ? AND ? OR t.complete_time <= 0) AND NOT EXISTS (SELECT 1 FROM subscription_orders AS s WHERE s.trade_no = t.trade_no AND s.status = ?)
 UNION ALL
 SELECT s.user_id, 0, 0, s.settlement_currency, s.payment_method, s.payment_provider,
  s.complete_time
 FROM subscription_orders AS s WHERE s.status = ? AND (s.complete_time BETWEEN ? AND ? OR s.complete_time <= 0) AND NOT EXISTS (SELECT 1 FROM subscription_payment_events AS e WHERE e.subscription_order_id = s.id)
 UNION ALL
 SELECT s.user_id, CASE WHEN e.created_time > 0 THEN e.settlement_amount_micros ELSE 0 END, 0, e.settlement_currency, s.payment_method, e.payment_provider, e.created_time
 FROM subscription_payment_events AS e JOIN subscription_orders AS s ON s.id = e.subscription_order_id WHERE s.status = ? AND (e.created_time BETWEEN ? AND ? OR e.created_time <= 0)
 UNION ALL
 ...
 `, common.TopUpStatusSuccess, from, now, common.TopUpStatusSuccess, common.TopUpStatusSuccess, from, now, common.TopUpStatusSuccess, from, now, FinanceEntryRevenue, FinanceSourceRefund, from, now)
	...
	err = cohort().Joins("JOIN (?) AS f ON f.user_id = u.id AND (f.occurred_at <= 0 OR (f.occurred_at >= u.created_at AND f.occurred_at <= ?))", events, now).
...
func PurgeAcquisition(ctx context.Context) error {
	if err := PurgeAcquisitionCorrections(ctx); err != nil {
		return err
	}
	if err := DB.WithContext(ctx).Where("snapshotted_at < ? OR user_id NOT IN (SELECT id FROM users WHERE deleted_at IS NULL)", time.Now().Unix()-AcquisitionAccountDays*86400).Delete(&AcquisitionFirstPayment{}).Error; err != nil {
		return err
	}
