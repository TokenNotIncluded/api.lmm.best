...
func TestBackendJourneyL0FilterMatchesLiveActivationPolicy(t *testing.T) {
 ...
 t.Cleanup(func(){common.QuotaPerUnit=oldQuota; *settings=oldSettings; SetLocalAcceptanceDeveloperAccess(oldLocal)})
 ...
 for i:=range users {
  users[i].Password="unused"; users[i].AffCode=users[i].Username; users[i].Status=common.UserStatusEnabled
  if users[i].Role==0 {users[i].Role=common.RoleCommonUser}
 ...
 for i:=range orders {
  orders[i].TradeNo=fmt.Sprintf("journey-%d",i); orders[i].Money=1
  if orders[i].Status=="" {orders[i].Status=common.TopUpStatusSuccess}
  if orders[i].PaymentProvider=="" {orders[i].PaymentProvider=PaymentProviderStripe}
  require.NoError(t,db.Create(&orders[i]).Error)
 }
 for _,tc:=range []struct{name string; enabled bool; minimum float64}{
  ...
  t.Run(tc.name,func(t *testing.T){
   ...
   for _,user:=range users {
    ...
    require.NoError(t,err)
    if user.Role<common.RoleAdminUser && !access.Granted {expected=append(expected,user.Id)}
   }
   var count int64
...
func TestBackendJourneyL0ThresholdRoundsTheCumulativeCredit(t *testing.T){
 ...
 oldLocal:=LocalAcceptanceDeveloperAccessEnabled()
 SetLocalAcceptanceDeveloperAccess(false)
 t.Cleanup(func(){*settings=oldSettings;common.QuotaPerUnit=oldQuota;SetLocalAcceptanceDeveloperAccess(oldLocal)})
 ...
 for _,stage:=range []string{"records","today","total count","total quota"}{
  t.Run(stage,func(t *testing.T){
   ...
   require.NoError(t,db.Callback().Query().Before("gorm:query").Register("journey:query-failure",func(tx *gorm.DB){
    if tx.Statement.Table=="checkins" {seen++;if seen==queryNumber {tx.AddError(injected)}}
   }))
   require.NoError(t,db.Callback().Row().Before("gorm:row").Register("journey:row-failure",func(tx *gorm.DB){
    if stage=="total quota" && tx.Statement.Table=="checkins" {tx.AddError(injected)}
   }))
