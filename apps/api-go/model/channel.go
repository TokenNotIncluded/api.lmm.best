...
func cloneChannelPointer[T any](value *T) *T {
	if value == nil {
		return nil
	}
	cloned := *value
...
func cloneChannelInfo(info ChannelInfo) ChannelInfo {
	info.MultiKeyStatusList = cloneIntMap(info.MultiKeyStatusList)
	info.MultiKeyDisabledReason = cloneIntStringMap(info.MultiKeyDisabledReason)
	info.MultiKeyDisabledTime = cloneInt64Map(info.MultiKeyDisabledTime)
	return info
}
...
func cloneChannel(channel *Channel) *Channel {
	...
	cloned := *channel
	cloned.OpenAIOrganization = cloneChannelPointer(channel.OpenAIOrganization)
	cloned.TestModel = cloneChannelPointer(channel.TestModel)
	cloned.Weight = cloneChannelPointer(channel.Weight)
	cloned.BaseURL = cloneChannelPointer(channel.BaseURL)
	cloned.ModelMapping = cloneChannelPointer(channel.ModelMapping)
	cloned.StatusCodeMapping = cloneChannelPointer(channel.StatusCodeMapping)
	cloned.Priority = cloneChannelPointer(channel.Priority)
	cloned.AutoBan = cloneChannelPointer(channel.AutoBan)
	cloned.Tag = cloneChannelPointer(channel.Tag)
	cloned.Setting = cloneChannelPointer(channel.Setting)
	cloned.ParamOverride = cloneChannelPointer(channel.ParamOverride)
	cloned.HeaderOverride = cloneChannelPointer(channel.HeaderOverride)
	cloned.Remark = cloneChannelPointer(channel.Remark)
...
func (c ChannelInfo) Value() (driver.Value, error) {
	...
	// pgx simple protocol encodes []byte as bytea, not JSON text.
...
func (c *ChannelInfo) Scan(value interface{}) error {
	...
	case []byte:
		return common.Unmarshal(value, c)
	case string:
		return common.Unmarshal([]byte(value), c)
	case nil:
		*c = ChannelInfo{}
		return nil
	default:
...
func (channel *Channel) GetNextEnabledKey() (string, int, *types.NewAPIError) {
	...
	runtimeState := getChannelRuntimeState(channel.Id, channel.ChannelInfo.MultiKeyPollingIndex)
	...
	case constant.MultiKeyModePolling:
		...
		for i := 0; i < len(keys); i++ {
			...
			if getStatus(idx) == common.ChannelStatusEnabled {
				...
				runtimeState.pollingIndex = (idx + 1) % len(keys)
				if common.DebugEnabled {
					logger.LogDebug(nil, "channel %d polling index: %d", channel.Id, runtimeState.pollingIndex)
				}
				if !common.MemoryCacheEnabled {
					channel.ChannelInfo.MultiKeyPollingIndex = runtimeState.pollingIndex
...
func BatchDeleteChannels(ids []int) (int64, error) {
	...
	for _, chunk := range lo.Chunk(ids, 200) {
		actualIDs, count, err := deleteChannelsReturningIDs(tx, "id in (?)", chunk)
		if err != nil {
			tx.Rollback()
			return 0, err
		}
	...
	cacheDeleteChannels(deletedIDs)
	return deletedCount, nil
}

func supportsMutationReturning() bool {
	return common.UsingMainDatabase(common.DatabaseTypePostgreSQL) || common.UsingMainDatabase(common.DatabaseTypeSQLite)
}

func deleteChannelsReturningIDs(tx *gorm.DB, where string, args ...any) ([]int, int64, error) {
	if supportsMutationReturning() {
		var deleted []Channel
		result := tx.Clauses(clause.Returning{Columns: []clause.Column{{Name: "id"}}}).Where(where, args...).Delete(&deleted)
		if result.Error != nil {
			return nil, 0, result.Error
		}
		ids := make([]int, 0, len(deleted))
		for _, channel := range deleted {
	...
	var ids []int
	if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Model(&Channel{}).Where(where, args...).Pluck("id", &ids).Error; err != nil {
		return nil, 0, err
	}
	if len(ids) == 0 {
		return nil, 0, nil
	...
	return ids, result.RowsAffected, result.Error
}

func updateChannelStatusReturningIDs(tx *gorm.DB, status int, where string, args ...any) ([]int, error) {
	if supportsMutationReturning() {
		var updated []Channel
		result := tx.Model(&updated).
...
func updateChannelStatus(channelId int, usingKey string, status int, reason string, recoveryIndex *int) bool {
	// Status serialization must also work without the memory cache (and while
	// cache refreshes prune the per-channel runtime locks).
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		// PostgreSQL/MySQL serialize competing processes and manual UPDATEs here.
		// SQLite omits FOR UPDATE; its transaction rejects conflicting writes.
...
func DisableChannelByTag(tag string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		var err error
		channelIDs, err = updateChannelStatusReturningIDs(tx, common.ChannelStatusManuallyDisabled, "tag = ?", tag)
		if err != nil {
			return err
		}
	...
	cacheDeleteChannels(channelIDs)
	return nil
}
...
func DeleteChannelByStatus(status int64) (int64, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		var err error
		channelIDs, deletedCount, err = deleteChannelsReturningIDs(tx, "status = ?", status)
		return err
	})
	if err != nil {
		return 0, err
...
func DeleteDisabledChannel() (int64, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		var err error
		channelIDs, deletedCount, err = deleteChannelsReturningIDs(
			tx,
			"status = ? or status = ?",
