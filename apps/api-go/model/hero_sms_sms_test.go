...
func TestHeroSMSSMSConcurrentIdempotentRetryPurchasesOnce(t *testing.T) {
	db := setupHeroSMSTestDB(t)
	createHeroSMSTestUser(t, db, 804, 2_000_000)
	require.NoError(t, UpdateHeroSMSSettings(HeroSMSSettingsUpdate{
		Enabled:         ptrBool(true),
		SMSEnabled:      ptrBool(true),
		APIKey:          "test-secret-key-12345",
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "getNumberV2":
			purchaseCalls.Add(1)
			_, _ = writer.Write([]byte(`{"activationId":912,"phoneNumber":"79001112233","activationCost":1,"currencyCode":643,"countryCode":6,"canGetAnotherSms":false}`))
		default:
			http.Error(writer, "unexpected action", http.StatusBadRequest)
		}
	}))
...
func TestHeroSMSSMSCancellationCannotRefundCompletedRace(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "getActiveActivations":
			_, _ = writer.Write([]byte(`{"data":[]}`))
		case "getNumberV2":
			_, _ = writer.Write([]byte(`{"activationId":913,"phoneNumber":"79002223344","activationCost":0.5,"currencyCode":643,"countryCode":6,"canGetAnotherSms":false}`))
		case "getStatusV2":
...
func TestHeroSMSSMSCancellationPendingRecoversInReconciliation(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "getActiveActivations":
			_, _ = writer.Write([]byte(`{"data":[]}`))
		case "getNumberV2":
			_, _ = writer.Write([]byte(`{"activationId":914,"phoneNumber":"79003334455","activationCost":0.5,"currencyCode":643,"countryCode":6,"canGetAnotherSms":false}`))
		case "getStatusV2":
			_, _ = writer.Write([]byte(`{"sms":null}`))
		case "setStatus":
