...
func(p *Provider) BrowserEntryHandler()http.Handler {
 return http.HandlerFunc(func(w http.ResponseWriter,r *http.Request){
  w.Header().Set("Cache-Control","no-store");w.Header().Set("X-Content-Type-Options","nosniff");w.Header().Set("Referrer-Policy","strict-origin")
  if r.Method!="GET"{p.Handler().ServeHTTP(w,r);return}
  ...
  if r.URL.Path=="/api/user/auth/oidc/authorize" {
   query,err:=url.ParseQuery(r.URL.RawQuery);if err!=nil{fail(w,400,"invalid_request");return}
   if _,err=p.parseAuthorization(query);err!=nil{fail(w,400,"invalid_request");return}
  }else if r.URL.Path!="/api/user/auth/oidc/grants"{http.NotFound(w,r);return}
  identity,err:=p.config.BrowserIdentity(ctx,r)
  if err==nil&&p.config.ValidateIdentity(ctx,identity)==nil{p.Handler().ServeHTTP(w,r);return}
