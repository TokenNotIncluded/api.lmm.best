...
func TestTopUpQuotaValidation(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:        "currency amount above legacy int32 limit",
			displayType: operation_setting.QuotaDisplayTypeUSD,
			amount:      4295,
...
func TestTopUpQuotaValidationRejectsNonFiniteQuotaPerUnit(t *testing.T) {
	oldQuotaPerUnit := common.QuotaPerUnit
	t.Cleanup(func() { common.QuotaPerUnit = oldQuotaPerUnit })

	for _, quotaPerUnit := range []float64{math.NaN(), math.Inf(1), math.Inf(-1), 0, -1} {
		common.QuotaPerUnit = quotaPerUnit
		_, err := getTopUpQuota(1)
		require.Error(t, err)
		assert.Zero(t, getMaxTopUpAmount())
	}
}
