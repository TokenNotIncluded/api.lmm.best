...
func TestBackendJourneyRegistrationRollsBackInitialTokenFailureAndCanRetry(t *testing.T){
 ...
 payment:=operation_setting.GetPaymentSetting();oldPayment:=*payment
 ...
 t.Cleanup(func(){constant.GenerateDefaultToken=oldDefault;setting.DefaultUseAutoGroup=oldAuto;common.QuotaForNewUser=oldQuota;*payment=oldPayment})
 ...
 require.NoError(t,db.Callback().Create().Before("gorm:create").Register("journey:token-failure",func(tx *gorm.DB){
  if failToken && tx.Statement.Table=="tokens" {tx.AddError(errors.New("injected initial token failure"))}
 }))
 register:=func()*httptest.ResponseRecorder{
  r:=httptest.NewRecorder();c,_:=gin.CreateTestContext(r)
...
func TestBackendJourneyRegistrationWithoutDefaultToken(t *testing.T){
 db:=setupManageUserTestDB(t)
 setRegistrationGateTestState(t,"terms","privacy")
 old:=constant.GenerateDefaultToken;constant.GenerateDefaultToken=false
...
func TestBackendJourneyTokenEditRejectsUnavailableGroupsWithoutBreakingRevocation(t *testing.T){
 for _,tc:=range []struct{name,group string;noRatio,revoked,statusOnly,success bool}{
  ...
  t.Run(tc.name,func(t *testing.T){
   ...
   if tc.noRatio {require.NoError(t,ratio_setting.UpdateGroupRatioByJSONString(`{"default":1}`))}
   if tc.revoked {require.NoError(t,setting.UpdateUserUsableGroupsByJSONString(`{"default":"Default"}`))}
   ...
   if tc.statusOnly {token.Group=tc.group}
   ...
   target:="/api/token/"
   if tc.statusOnly {target+="?status_only=true";request["status"]=common.TokenStatusDisabled}
   ...
   if !tc.success {assert.Equal(t,token,stored,"failed validation must not persist any partial edit")}
   if tc.statusOnly {assert.Equal(t,common.TokenStatusDisabled,stored.Status)}
  })
 }
}
