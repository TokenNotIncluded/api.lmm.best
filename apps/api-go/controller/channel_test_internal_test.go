...
func TestValidateChannelRequiresNewAPIBaseURL(t *testing.T) {
	tests := []struct {
		...
		{name: "configured", baseURL: common.GetPointer("https://relay.example")},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.wantErr {
				require.ErrorContains(t, err, "compatible relay channel base URL cannot be empty")
				return
			}
			require.NoError(t, err)
		})
...
func TestNewAPIChannelRegistration(t *testing.T) {
	...
	assert.Equal(t, "Compatible Relay", constant.GetChannelTypeName(constant.ChannelTypeNewAPI))
...
func TestResponsesCompactChannelSupport(t *testing.T) {
	tests := []struct {
		...
		{name: "Compatible Relay", channelType: constant.ChannelTypeNewAPI, apiType: constant.APITypeNewAPI, want: true},
