...
- Initial `scope`: `catalog:read balance:read usage:read models:invoke mcp:bounties mcp:drawing` (space-separated). `usage:read` permits the native client's own daily activity aggregate; it never exposes request content or other accounts. The two `mcp:*` scopes authorize the built-in `/mcp` and `/mcp/drawing` servers in the same login. No secret, OIDC, DPoP or dynamic registration.
