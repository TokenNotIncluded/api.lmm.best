# LMM Pi OAuth HTTP profile (stage 2, disabled by default)
...
- Public native `client_id`: `lmm-pi`; display name: `LMM for Pi`.
...
- Initial `scope`: `catalog:read balance:read models:invoke` (space-separated). No secret, OIDC, DPoP or dynamic registration.
