...
- Initial `scope`: `catalog:read balance:read models:invoke mcp:bounties mcp:drawing` (space-separated). The two `mcp:*` scopes authorize the built-in `/mcp` and `/mcp/drawing` servers in the same login. No secret, OIDC, DPoP or dynamic registration.
