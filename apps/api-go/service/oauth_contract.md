...
- Public native clients: `lmm-pi` (`LMM for Pi`), `lmm-dsh` (`LMM for DSH`) and `lmm` (`LMM CLI`). Token families remain bound to the client that created them.
- The `lmm` CLI has a separate **read-only** initial profile: `catalog:read balance:read`. Its registry excludes model invocation, usage history, MCP and account/application management. Consent adds only the same explicitly displayed group snapshot described below. CLI consent does not display the model-invocation permission. This registration alone does not implement device-code login, app grants, spending limits or third-party setup, and does not establish production deployment.
...
- Pi/DSH current initial `scope`: `catalog:read balance:read usage:read models:invoke mcp:bounties mcp:drawing` (space-separated). `usage:read` permits the native client's own daily activity aggregate; it never exposes request content or other accounts. The two `mcp:*` scopes authorize the built-in `/mcp` and `/mcp/drawing` servers in the same login. No secret, OIDC, DPoP or dynamic registration.
