...
func (g *generator) refine(name string, c *contract) {
	...
	case "UpdateOption":
		...
		c.notes = append(c.notes, "value is converted to a string by this route. For JSON-valued options such as ModelRatio, ModelPrice and CompletionRatio, send a JSON-encoded string, never a nested object. Read existing option values, merge only requested model entries, then validate with ValidateOptions and write related options with UpdateOptionsBulk. ModelPriceLock defaults to an empty JSON object (all models unlocked). Locked model pricing changes are ignored with warnings while the request succeeds; read warnings and the effective values before reporting a change as applied.")
	case "UpdateOptionsBulk", "ValidateOptions":
		c.body["required"] = []string{"values"}
		c.notes = append(c.notes, "values is a non-empty map of option keys to STRING values, with at most 128 entries. JSON settings must be JSON-encoded strings. Read and merge existing pricing maps before replacement. ValidateOptions performs the same option validation without persisting; UpdateOptionsBulk writes the complete related set in one database transaction. Locked model prices, ratios and billing expressions are preserved before validation. Inspect warnings and effective values: a successful response may have ignored locked entries. Unlock separately before editing a locked price.")
