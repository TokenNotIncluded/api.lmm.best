...
func TestBackendJourneyL0CheckoutSettlementKeyAndDailyCheckin(t *testing.T){
 ...
 access:=operation_setting.GetDeveloperAccessSetting();oldAccess:=*access
 checkin:=operation_setting.GetCheckinSetting();oldCheckin:=*checkin
 ...
 *checkin=operation_setting.CheckinSetting{Enabled:true,MinQuota:10,MaxQuota:10,LevelMultipliers:[]float64{1,1,1,1,1}}
 t.Cleanup(func(){
  ...
  common.QuotaPerUnit=oldQuota;model.SetLocalAcceptanceDeveloperAccess(oldLocal)
 ...
 require.NoError(t,db.Create(&user).Error);require.NoError(t,db.Create(&other).Error)
 bundle,err:=service.CreateLoginSession(user.Id,"password","192.0.2.98","backend-journey-test")
 require.NoError(t,err)
 gin.SetMode(gin.TestMode)
 engine:=gin.New();SetApiRouter(engine)
 request:=func(method,path,body string,authenticated bool)*httptest.ResponseRecorder{
  ...
  if authenticated {req.Header.Set("Authorization","Bearer "+bundle.AccessToken)}
  res:=httptest.NewRecorder();engine.ServeHTTP(res,req);return res
 }
 decode:=func(res *httptest.ResponseRecorder)map[string]interface{}{
  var payload map[string]interface{}
  require.NoError(t,json.Unmarshal(res.Body.Bytes(),&payload),res.Body.String());return payload
 }
 ...
 require.NoError(t,db.Create(&order).Error);require.NoError(t,db.Create(&foreign).Error)
 ...
 for i:=0;i<2;i++ {_,err:=model.CompleteExternalTopUp(evidence);require.NoError(t,err)}
