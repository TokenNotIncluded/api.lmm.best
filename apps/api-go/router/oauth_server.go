...
func SetOAuthServerRouter(router *gin.Engine) error {
 ...
 if err != nil { return fmt.Errorf("configure OAuth server: %w", err) }
 integration, err := service.ConfigureOAuthIntegration(model.DB, config)
 if err != nil { return fmt.Errorf("initialize OAuth server: %w", err) }
 MountOAuthServerRoutes(router, integration)
 if err := mountSubprojectOIDC(router); err != nil { return fmt.Errorf("initialize subproject OIDC: %w", err) }
 return nil
}
