...
func TestConvertRequestResponsesToGeminiUsesDirectConverter(t *testing.T) {
	...
	require.Nil(t, functions[0].Parameters)
	params, ok := functions[0].ParametersJsonSchema.(map[string]any)
	require.True(t, ok)
	assert.Equal(t, "object", params["type"])
	assert.Contains(t, params, "additionalProperties")
	...
	assert.Contains(t, queryParam, "exclusiveMinimum")
	filterParam, ok := properties["filters"].(map[string]any)
	require.True(t, ok)
	filterItems, ok := filterParam["items"].(map[string]any)
	require.True(t, ok)
	assert.Contains(t, filterItems, "additionalProperties")
