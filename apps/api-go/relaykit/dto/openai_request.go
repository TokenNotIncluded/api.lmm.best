...
func (r GeneralOpenAIRequest) MarshalJSON() ([]byte, error) {
	...
	// Encode messages once with the request instead of allocating an encoded
	// RawMessage for every history item (which may contain large media payloads).
...
func GetOpenAIChatCapabilities(modelName, reasoningEffort string) OpenAIChatCapabilities {
	...
	// without reasoning. GPT-6 Sol and Luna default to medium, so sampling is
	// retained only when the caller explicitly selects none. GPT-6 Astra never
	...
	supportsSampling := isGPT6SolLuna && reasoningEffort == "none"
...
type Message struct {
	...
	// Tools preserves dynamic tool declarations on OpenAI-compatible messages.
