...
// exceedsMaxCharacters matches the UTF-16 code-unit semantics used by the
// browser console when enforcing these limits. Counting bytes rejects normal
// CJK text too early, while counting runes lets surrogate-pair emoji exceed the
// limit accepted by the UI.
func exceedsMaxCharacters(value string, max int) bool {
