...
#[tokio::test]
async fn gift_discovery_requires_dashboard_auth_before_database_access() {
        ...
        PgValkeyDashboardAuth::new(
            ...
            AuthConfig {
                session_secret: SecretString::from(
                    "compensation-gift-route-test-secret-012345678901234567890123456789",
                ),
                ..AuthConfig::default()
            },
        )
    ...
    let app = router(GiftState::new(pg, valkey, auth));

    let response = app
        .oneshot(
            Request::get("/api/user/gift")
                .body(Body::empty())
