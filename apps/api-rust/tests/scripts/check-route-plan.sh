...
legacy="${ROUTE_LEGACY_PATH:-$repo_root/apps/api-rust/tests/fixtures/routes/legacy-go-routes.tsv}"
plan="${ROUTE_PLAN_PATH:-$repo_root/apps/api-rust/tests/fixtures/routes/route-plan.tsv}"
gate="${ROUTE_GATE_PATH:-$repo_root/apps/api-rust/tests/fixtures/routes/route-gate.tsv}"
review="${ROUTE_INTEGRATION_REVIEW_PATH:-$repo_root/apps/api-rust/tests/fixtures/routes/integration-review.tsv}"
frozen_contract="${ROUTE_FROZEN_AUTH_PATH:-$repo_root/apps/api-rust/tests/fixtures/routes/frozen-route-auth.tsv}"
...
gate_validator="${ROUTE_GATE_VALIDATOR_PATH:-$repo_root/packaging/common/lmm-api/validate-route-gate}"
release_revision="${ROUTE_RELEASE_REVISION:-$(git -C "$repo_root" rev-parse HEAD 2>/dev/null || true)}"
evidence_root="${ROUTE_EVIDENCE_ROOT:-$repo_root}"
snapshot_dir=$(mktemp -d "$repo_root/.route-gate-snapshot.XXXXXXXX")
asset_manifest=$(mktemp "$repo_root/.route-gate-assets.XXXXXXXX")
cleanup() {
  rm -rf -- "$snapshot_dir"
  rm -f -- "$asset_manifest"
}
...
[[ $(tsv_first_line "$plan") == "$expected_header" ]] || {
  echo "invalid route-plan header" >&2
  exit 1
}
...
diff -u <(tsv_without_crlf "$frozen_contract") <(
  ...
  echo "frozen route/auth contract differs from route-plan membership or auth scope" >&2
  exit 1
}

[[ -f "$gate" ]] || {
  echo "missing route evidence gate: $gate" >&2
  exit 1
}
...
if rg -Fq $'/api/user/logout' <(tsv_without_crlf "$gate") <(tsv_without_crlf "$review"); then
  echo "obsolete /api/user/logout must not appear in route evidence" >&2
  exit 1
fi
...
while IFS=$'\t' read -r method path source_state compile_state mount_state differential_state approval_state owner gate_state evidence; do
  ...
  if [[ $route_stub_state == stub ]]; then
    if is_frozen_model_delete_501 "$method" "$path"; then
      ...
      : # Static legacy-equivalent 501 candidate; it earns no Rust route ownership credit.
    else
...
' <(tsv_without_crlf "$gate")) || {
  echo "route gate is missing POST /api/user/register" >&2
  exit 1
}
if [[ $registration_mount_state == mounted ]]; then
  normal_main="${ROUTE_MAIN_PATH:-$repo_root/apps/api-rust/src/main.rs}"
  ...
  [[ $registration_wiring_compact == *"letidentity_security=lmm_api_rs::routes::identity_security::router("* ]] || {
...
diff -u <(printf '%s\n' "$expected_root_mounts" | LC_ALL=C sort) <(printf '%s\n' "$actual_root_mounts") || {
  echo "route gate core root mount inventory must contain exactly 20 authorized local Rust routes" >&2
  exit 1
}
...
diff -u <(printf '%s\n' "$expected_blocked_routes" | sed '/^$/d' | LC_ALL=C sort) <(printf '%s\n' "$actual_blocked_routes" | sed '/^$/d' | LC_ALL=C sort) || {
  echo "route gate must block exactly the zero routes stopped by the latest independent Sol review" >&2
  exit 1
}
...
candidate_dir="$repo_root/apps/api-rust/src/routes"
candidate_mod="$repo_root/apps/api-rust/src/routes.rs"

# Some route modules are reusable executors for isolated listener fixtures
# rather than route factories. They must remain declared
...
for helper in legacy_http relay_anthropic_gemini_postgres relay_misc_postgres sse test_support; do
  [[ -f "$candidate_dir/$helper.rs" ]] || {
    echo "declared route helper is missing: $helper.rs" >&2
    exit 1
  }
  grep -Eq "^pub(\(crate\))? mod $helper;$" "$candidate_mod" || {
    echo "route helper is not declared: $helper" >&2
    exit 1
  }
done
...
root_router="${ROUTE_ROOT_PATH:-$repo_root/apps/api-rust/src/http.rs}"
[[ -f $root_router ]] || {
...
allowed_candidate_import='routes::api_token::{ApiTokenHttpState,ApiTokenPrincipal,api_token_router},'
...
if [[ $production_without_allowed_import == *"routes::"* ]]; then
