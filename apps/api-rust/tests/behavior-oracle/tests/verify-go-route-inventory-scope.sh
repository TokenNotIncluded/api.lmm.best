...
[[ "${ledger_count}" -eq 355 ]] ||
  fail "frozen ledger contains ${ledger_count} rows; expected 355"
...
if [[ -s "${extra_identities}" || -s "${missing_identities}" || -s "${extra_paths}" || -s "${missing_paths}" ]]; then
  echo "inventory exceeds route scope: authoritative Go registration is not exactly the frozen 355-route ledger" >&2
