...
use super::support::*;
use crate::migration_routes::assistant::key_creation::{KeyCreationError, confirm_pg};

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn pg_commit_time_authorization_fence_rejects_completed_security_mutations() {
    ...
    for case in [
        ...
        .execute(&store.pg)
        .await
        .expect("reset authoritative user state");
        ...
        sqlx::query("DELETE FROM two_fa_backup_codes")
            .execute(&store.pg)
            .await
            .expect("reset backup codes");
        sqlx::query("DELETE FROM two_fas")
            .execute(&store.pg)
            .await
            .expect("reset factors");
        ...
        if case == "rotate-backup-code" {
            ...
            .bind(hash(old_backup, DEFAULT_COST).expect("hash old backup code"))
            .execute(&store.pg)
            .await
            .expect("seed old backup code");
        }

        let mut mutation = store.pg.begin().await.expect("begin authority mutation");
        let blocker_pid = sqlx::query_scalar::<_, i32>("SELECT pg_backend_pid()")
            .fetch_one(&mut *mutation)
            .await
            .expect("read mutation backend pid");

        sqlx::query("SELECT id FROM users WHERE id = 7 FOR UPDATE")
            .execute(&mut *mutation)
            .await
            .expect("lock user before authority mutation");
        match case {
            "revoke-l1" => {
            ...
            "bump-user-auth-version" => {
                sqlx::query("UPDATE users SET auth_version = 2 WHERE id = 7")
                    .execute(&mut *mutation)
                    .await
                    .expect("bump user auth version");
            }
            "enable-two-factor" => {
                sqlx::query("UPDATE users SET auth_version = 2 WHERE id = 7")
                    .execute(&mut *mutation)
                    .await
                    .expect("invalidate pre-enrollment auth version");
                sqlx::query(
                    "INSERT INTO two_fas (user_id, secret, is_enabled, failed_attempts, updated_at) \
                     VALUES (7, 'JBSWY3DPEHPK3PXP', TRUE, 0, NOW())",
                )
                .execute(&mut *mutation)
                .await
                .expect("enable two-factor factor");
            }
            "rotate-backup-code" => {
                sqlx::query("SELECT id FROM two_fas WHERE user_id = 7 FOR UPDATE")
                    .execute(&mut *mutation)
                    .await
                    .expect("lock current factor");
                sqlx::query(
                    "SELECT id FROM two_fa_backup_codes WHERE user_id = 7 AND is_used = FALSE FOR UPDATE",
                )
                .execute(&mut *mutation)
                .await
                .expect("lock current backup generation");
                sqlx::query("DELETE FROM two_fa_backup_codes WHERE user_id = 7")
                    .execute(&mut *mutation)
                    .await
                    .expect("remove old backup generation");
                sqlx::query(
                    "INSERT INTO two_fa_backup_codes (user_id, code_hash, is_used) VALUES (7, $1, FALSE)",
                )
                .bind(hash("MNOP-QRST-UVWX", DEFAULT_COST).expect("hash new backup code"))
                .execute(&mut *mutation)
                .await
                .expect("install new backup generation");
            }
            _ => unreachable!("bounded authorization mutation case"),
        }
        ...
        let token = confirmation(&confirm_action);
        ...
        mutation
            .commit()
            .await
            .expect("commit authority mutation first");

        let error = confirmation_task
            .await
            .expect("join blocked confirmation")
            .expect_err("stale authorization fence must reject");
        if case == "rotate-backup-code" {
        ...
        .fetch_one(&store.pg)
        .await
        .expect("read authority flow consumption");
