...
#[tokio::test]
async fn pg_confirmation_is_session_bound_expiring_replay_safe_and_exactly_once() {
    ...
    for _ in 0..2 {
        ...
        tasks.push(tokio::spawn(async move {
            barrier.wait().await;
            confirm_pg(&store, authorization_fence("session-a"), token, "").await
        }));
    }
    barrier.wait().await;
    let first = tasks.remove(0).await.expect("first confirmation task");
    let second = tasks.remove(0).await.expect("second confirmation task");
...
#[tokio::test]
async fn pg_confirmation_revalidates_group_ratio_warning_and_user_state() {
    let Some(harness) = PgHarness::new().await else {
        eprintln!("skipping assistant-key PostgreSQL test: LMM_TEST_DATABASE_URL is unset");
        return;
    };
    ...
    .execute(&harness.pool)
    .await
    .expect("remove the prepared selectable group");
    assert_eq!(
...
#[tokio::test]
async fn pg_zero_ratio_warning_matches_go_default_and_explicit_disable_wins() {
    let Some(harness) = PgHarness::new().await else {
        eprintln!("skipping assistant-key PostgreSQL test: LMM_TEST_DATABASE_URL is unset");
        return;
    };
    ...
    sqlx::query("UPDATE options SET value = '{\"default\":0,\"vip\":2}' WHERE key = 'GroupRatio'")
        .execute(&harness.pool)
        .await
        .expect("set zero group ratio");
    let options = load_pg_options(&store, "default")
        .await
        .expect("load zero-ratio warning");
    let warning = options
        .iter()
        .find(|option| option.id() == "default")
        .and_then(|option| option.warning.as_ref())
        .expect("zero ratio receives the Go-compatible warning");
    ...
    sqlx::query("UPDATE options SET value = '{\" DEFAULT \" : {\"enabled\":false,\"message\":\"disabled\",\"mode\":\"modal\",\"confirmations\":3}}' WHERE key = 'group_ratio_setting.group_warnings'")
        .execute(&harness.pool)
        .await
        .expect("explicitly disable zero-ratio warning");
    let options = load_pg_options(&store, "default")
        .await
        .expect("reload explicitly disabled warning");
    assert!(
        options
            .iter()
            .find(|option| option.id() == "default")
