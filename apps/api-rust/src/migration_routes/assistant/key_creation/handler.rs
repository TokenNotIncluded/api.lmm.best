...
pub(in crate::migration_routes::assistant) async fn prepare_key_handler(
...
pub(in crate::migration_routes::assistant) async fn confirm_key_handler(
    State(state): State<AssistantReadState>,
    request: axum::extract::Request,
) -> Response {
    let headers = request.headers().clone();
...
pub(in crate::migration_routes::assistant) async fn prepare_tool(
    state: &AssistantReadState,
