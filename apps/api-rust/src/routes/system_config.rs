...
/// Authentication boundary supplied by dashboard auth.
///
    ...
    async fn authorize_root(
        ...
        self.require_root_dashboard_session(headers)
            ...
            .map_err(|_| SystemConfigAuthRejection::Unauthorized {
                supplied: crate::routes::legacy_http::dashboard_credential(headers).is_some(),
            })
    }
}
...
/// The listener must have one explicit owner for that second step; writing
/// PostgreSQL alone would report success while the
...
impl Error for ProtocolRolloutRuntimeError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            ...
            Self::InvalidOption { .. } | Self::PublicCatalog => None,
        }
    }
}
...
async fn get_setup(State(state): State<SystemConfigHttpState>) -> Response {
    ...
    if root_init
        && sqlx::query("INSERT INTO setups (version, initialized_at) VALUES ('lmm-api-rs', $1)")
            .bind(chrono_seconds())
            .execute(&state.pg)
            .await
            .is_ok()
...
async fn post_setup(State(state): State<SystemConfigHttpState>, body: Bytes) -> Response {
    ...
    // linearizable without changing the shared schema.
    ...
    if sqlx::query("INSERT INTO setups (version, initialized_at) VALUES ('lmm-api-rs', $1)")
        .bind(chrono_seconds())
        .execute(&mut *tx)
        .await
        .is_err()
