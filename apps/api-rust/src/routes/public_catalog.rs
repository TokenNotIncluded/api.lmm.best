//! Public catalogue, pricing, rankings, and token-usage routes.
//!
//! Every value comes from an injected durable store, and authentication is
//! server-derived. This module performs no outbound provider requests.
...
pub struct PublicCatalogPrincipal {
    pub user_id: i64,
    pub role: i64,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PublicCatalogAuthError {
    UnmatchedOpaque,
    Unauthorized,
    TokenExpired,
    SessionRevoked,
    ...
    PolicyInvalid {
        principal: PublicCatalogPrincipal,
...
#[async_trait]
pub trait PublicCatalogAuthorizer: Send + Sync {
    async fn principal(
        &self,
        headers: &HeaderMap,
...
#[derive(Clone)]
pub struct DashboardPublicCatalogAuthorizer {
    auth: Arc<dyn DashboardAuth>,
}

impl DashboardPublicCatalogAuthorizer {
    #[must_use]
...
#[async_trait]
impl PublicCatalogAuthorizer for DashboardPublicCatalogAuthorizer {
    async fn principal(
        &self,
        headers: &HeaderMap,
    ) -> Result<PublicCatalogPrincipal, PublicCatalogAuthError> {
        let token = crate::routes::legacy_http::dashboard_credential(headers)
            .ok_or(PublicCatalogAuthError::Unauthorized)?;
        let internal = dashboard_token_candidate(&token);
        match self.auth.self_user(SecretString::from(token)).await {
            Ok(user) => {
                let principal = PublicCatalogPrincipal {
                    user_id: user.id,
                    role: user.role,
                };
                match enforce_user_auth(&user) {
                    Ok(()) => Ok(principal),
                    Err(error) => Err(PublicCatalogAuthError::UserAuth(error)),
                }
            }
            Err(error) if error.kind == AuthErrorKind::TokenExpired => {
                Err(PublicCatalogAuthError::TokenExpired)
            }
            Err(error) if error.kind == AuthErrorKind::SessionRevoked => {
                Err(PublicCatalogAuthError::SessionRevoked)
            }
            Err(error)
                if matches!(
                    error.kind,
                ...
                ) =>
            {
                Err(if internal {
                    ...
                    PublicCatalogAuthError::UnmatchedOpaque
                })
            }
            Err(error) if error.kind == AuthErrorKind::UserDisabled => Err(
                PublicCatalogAuthError::UserAuth(UserAuthPolicyError::UserDisabled),
            ),
            Err(_) => Err(PublicCatalogAuthError::Unavailable),
        }
    }
    ...
    async fn optional_principal(
            ...
            .await
        {
            Ok(user) => {
                ...
                match enforce_user_auth(&user) {
                    Ok(()) => Ok(principal),
                    Err(error) => Err(PublicCatalogAuthError::PolicyInvalid { principal, error }),
                }
            }
            Err(error) if error.kind == AuthErrorKind::TokenExpired => {
                Err(PublicCatalogAuthError::TokenExpired)
...
pub struct PublicCatalogStoreError(pub String);

impl PublicCatalogStoreError {
    #[must_use]
...
impl std::fmt::Display for PublicCatalogStoreError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str(&self.0)
    }
}

impl std::error::Error for PublicCatalogStoreError {}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct PublicCatalogToken {
    pub user_id: i64,
    pub status: i64,
    pub user_status: i64,
...
#[async_trait]
pub trait PublicCatalogStore: Send + Sync {
...
#[async_trait]
pub trait PublicCatalogRateLimiter: Send + Sync {
    async fn check(
        &self,
        client_ip: &str,
...
#[derive(Clone)]
pub struct DashboardPublicCatalogRateLimiter {
    auth: Arc<dyn DashboardAuth>,
}

impl DashboardPublicCatalogRateLimiter {
    #[must_use]
    pub fn new(auth: Arc<dyn DashboardAuth>) -> Self {
        Self { auth }
    }
...
#[async_trait]
impl PublicCatalogRateLimiter for DashboardPublicCatalogRateLimiter {
    async fn check(
        &self,
        client_ip: &str,
    ) -> Result<CriticalRateLimitOutcome, PublicCatalogStoreError> {
...
struct AllowPublicCatalogRateLimiter;

#[async_trait]
impl PublicCatalogRateLimiter for AllowPublicCatalogRateLimiter {
...
/// State for [`public_catalog_router`].
#[derive(Clone)]
pub struct PublicCatalogState {
...
impl PublicCatalogState {
    ...
    async fn pricing(&self, actor: Option<PublicCatalogPrincipal>) -> Value {
    ...
    fn read_last_good(&self) -> std::sync::RwLockReadGuard<'_, PublicCatalogLastGood> {
        self.last_good
            .read()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
    }

    fn write_last_good(&self) -> std::sync::RwLockWriteGuard<'_, PublicCatalogLastGood> {
        self.last_good
            .write()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
    }
...
///
/// The broader [`PublicCatalogState`] remains isolated on the
...
impl RatioConfigState {
    ...
    pub fn new(pg: PgPool, auth: Arc<dyn DashboardAuth>) -> Self {
        Self {
            pg,
            limiter: Arc::new(DashboardPublicCatalogRateLimiter::new(auth)),
        }
    }
}
...
impl GroupState {
    ...
    pub fn new(pg: PgPool, auth: Arc<dyn DashboardAuth>) -> Self {
        Self {
            pg,
            authorizer: Arc::new(DashboardPublicCatalogAuthorizer::new(auth)),
        }
    }
}
...
/// Builds the public catalogue routes.
pub fn public_catalog_router(state: PublicCatalogState) -> Router {
...
async fn console_access_boundary(
    ...
    let Some(token) = crate::routes::legacy_http::dashboard_credential(request.headers()) else {
...
async fn groups(State(state): State<PublicCatalogState>, headers: HeaderMap) -> Response {
...
async fn models(State(state): State<PublicCatalogState>, headers: HeaderMap) -> Response {
...
async fn pricing(State(state): State<PublicCatalogState>, headers: HeaderMap) -> Response {
...
async fn rankings(
    ...
    let response = match state.store.rankings(&period).await {
        ...
        Err(PublicCatalogStoreError(message)) => public_failure(StatusCode::BAD_REQUEST, &message),
    };
    if actor.is_some() {
        with_auth_version(response)
    } else {
...
async fn ratio_config(State(state): State<PublicCatalogState>) -> Response {
...
async fn token_usage_get(State(state): State<PublicCatalogState>, request: Request) -> Response {
...
async fn token_usage(state: &PublicCatalogState, headers: &HeaderMap) -> Response {
...
async fn nav_actor(
    ...
    module: &str,
) -> Result<Option<PublicCatalogPrincipal>, Response> {
    if !access.enabled {
    ...
    match authorization {
        ...
        Err(PublicCatalogAuthError::UnmatchedOpaque) if !access.require_auth => Ok(None),
        Err(PublicCatalogAuthError::UnmatchedOpaque | PublicCatalogAuthError::Unauthorized) => {
            Err(public_dashboard_unauthorized(headers))
        }
        Err(PublicCatalogAuthError::TokenExpired) => {
            Err(dashboard_auth_failure(headers, AuthErrorKind::TokenExpired))
        }
        Err(PublicCatalogAuthError::SessionRevoked) => Err(dashboard_auth_failure(
            headers,
            AuthErrorKind::SessionRevoked,
        )),
        Err(PublicCatalogAuthError::Unavailable) => Err(dashboard_internal_error(headers)),
        Err(PublicCatalogAuthError::UserAuth(error)) => Err(public_user_auth_error(headers, error)),
        Err(PublicCatalogAuthError::PolicyInvalid { principal, .. }) if !access.require_auth => {
            Ok(Some(principal))
        }
        Err(PublicCatalogAuthError::PolicyInvalid { error, .. }) => {
...
async fn require_public_dashboard_authorizer(
    ...
    match authorizer.principal(headers).await {
        ...
        Err(PublicCatalogAuthError::Unavailable) => Err(dashboard_internal_error(headers)),
        Err(PublicCatalogAuthError::UserAuth(error)) => Err(public_user_auth_error(headers, error)),
        Err(PublicCatalogAuthError::PolicyInvalid { .. }) => {
...
async fn require_dashboard(
    ...
    match state.authorizer.principal(headers).await {
        ...
        Err(PublicCatalogAuthError::Unavailable) => Err(dashboard_internal_error(headers)),
        Err(PublicCatalogAuthError::UserAuth(error)) => Err(user_auth_error(headers, error)),
        Err(PublicCatalogAuthError::PolicyInvalid { error, .. }) => {
...
async fn require_assistant_dashboard(
    ...
    match state.authorizer.browser_session_principal(headers).await {
        ...
        Err(PublicCatalogAuthError::TokenExpired) => Err(with_auth_version(
            dashboard_auth_failure(headers, AuthErrorKind::TokenExpired),
        )),
        Err(PublicCatalogAuthError::SessionRevoked) => Err(with_auth_version(
            dashboard_auth_failure(headers, AuthErrorKind::SessionRevoked),
        )),
        Err(PublicCatalogAuthError::Unavailable) => {
            Err(with_auth_version(dashboard_internal_error(headers)))
        }
        Err(PublicCatalogAuthError::UserAuth(error)) => {
...
fn dashboard_credential_present(headers: &HeaderMap) -> bool {
    crate::routes::legacy_http::dashboard_credential(headers).is_some()
}
...
#[async_trait]
impl PublicCatalogStore for MemoryPublicCatalogStore {
    async fn header_nav(&self, module: &str) -> Result<HeaderNavAccess, PublicCatalogStoreError> {
    ...
    async fn pricing(
        &self,
        _: Option<PublicCatalogPrincipal>,
    ) -> Result<Value, PublicCatalogStoreError> {
    ...
    async fn rankings(&self, period: &str) -> Result<Value, PublicCatalogStoreError> {
        ...
        self.ranking_data
            .get(period)
            .cloned()
            .ok_or_else(|| PublicCatalogStoreError::new(format!("missing ranking data: {period}")))
    }
    async fn exposed_ratio(&self) -> Result<Option<Value>, PublicCatalogStoreError> {
        Ok(self.ratio.clone())
    }
...
mod tests {
    ...
    struct Auth(Option<PublicCatalogPrincipal>);
    #[async_trait]
    impl PublicCatalogAuthorizer for Auth {
        async fn principal(
            &self,
            _: &HeaderMap,
        ) -> Result<PublicCatalogPrincipal, PublicCatalogAuthError> {
            self.0.ok_or(PublicCatalogAuthError::Unauthorized)
        }
    ...
    fn app(role: Option<i64>) -> Router {
        ...
        public_catalog_router(PublicCatalogState::new(
            Arc::new(store),
            Arc::new(Auth(
                role.map(|role| PublicCatalogPrincipal { user_id: 1, role }),
            )),
        ))
    }
    ...
    fn dashboard_credential_parser_matches_shared_http_semantics() {
        for (raw, expected) in [
            ...
            assert_eq!(
                crate::routes::legacy_http::dashboard_credential(&headers).as_deref(),
                expected,
                "{raw:?}"
            );
        }
    ...
    async fn normal_ratio_config_read_fails_closed_when_options_are_unavailable() -> TestResult {
        ...
        let response = ratio_config_direct(axum::extract::State(RatioConfigState {
            pg: pool,
            limiter: std::sync::Arc::new(AllowPublicCatalogRateLimiter),
        }))
        .await;
