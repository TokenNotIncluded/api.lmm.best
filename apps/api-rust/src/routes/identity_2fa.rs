...
//!
//! The listener mounts [`router`] behind its authenticated-user extractor and
//! supplies a [`SecuritySessionRotator`]. Keeping identity
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::routes) enum CriticalTwoFactorVerification {
    NotRequired,
    Verified,
    Rejected,
}

pub(in crate::routes) async fn verify_critical_mutation_factor(
