...
use crate::routes::assistant_extended;

mod key_creation;
#[cfg(test)]
pub(in crate::routes) mod pg_test_support;
mod secure_card;

pub(in crate::routes) fn decrypt_assistant_secure_card_payload(
    session_secret: &SecretString,
    ciphertext: &str,
) -> Option<serde_json::Value> {
    secure_card::decrypt_payload(session_secret, ciphertext).ok()
}

#[cfg(test)]
pub(in crate::routes) fn encrypt_assistant_secure_card_fixture(
    session_secret: &SecretString,
    plaintext: &[u8],
) -> Result<String, ()> {
    secure_card::encrypt_payload(session_secret, plaintext)
}
...
use crate::auth::{
    AuthErrorKind, CriticalRateLimitOutcome, DashboardAuth, DashboardDeveloperAccessPolicy,
...
use super::identity_catalog::user_group_selection;
...
- Use live tools for account state, model availability, pricing, discounts, invitation rewards, usage statistics, and search results. If a tool is unavailable, say so instead of inventing a value. Never write a model ID, version, or release name into a config example, command, or explanation unless it was returned verbatim by a live tool in this conversation; use the literal placeholder <MODEL_ID> instead of guessing one.\n\
...
fn assistant_tool_definitions() -> Vec<Value> {
    ...
    vec![
        ...
        tool(
            "request_create_key",
            "Prepare creation of an API key. First call without a group to load the signed-in user's live, real selectable groups, then ask the user to choose one exact group. The virtual auto group is never valid. If a warning is configured, collect the exact group_warning_confirmations. This creates only an opaque, session-bound draft; never claim a key exists before confirmation.",
            object(
                json!({
                    "name":{"type":"string","maxLength":50},
                    "group":{"type":"string","maxLength":64},
                    "conversation_id":{"type":"integer","minimum":0},
                    "group_warning_confirmations":{"type":"integer","minimum":0,"maximum":10}
                }),
                &[],
            ),
        ),
...
#[async_trait]
trait AssistantReadStore: Send + Sync + key_creation::Repository {
...
pub struct AssistantReadState {
    ...
    pub(crate) developer_access_policy: DashboardDeveloperAccessPolicy,
...
pub fn assistant_read_router(state: AssistantReadState) -> Router {
    Router::new()
        ...
        .route(
            ...
            post(key_creation::prepare_key_handler),
        )
        .route(
            "/api/assistant/tools/create-key",
            post(key_creation::confirm_key_handler),
        )
...
async fn execute_assistant_tool(
    ...
    if call.function.name.trim() == "request_create_key" {
        return key_creation::prepare_tool(state, actor, session_id, &input).await;
    }
