...
//!
//! `routes` must be merged behind the same token-auth, performance, rate-limit,
//! and channel-distribution adapters as legacy `SetRelayRouter`; it is not a
//! public router.
...
/// Relay formats handled by this module.
...
///
/// Route ownership is split between the `active` and `frozen` modules so
/// provider-capable paths and legacy unavailable endpoints are declared once.
