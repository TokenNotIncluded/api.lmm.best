...
#[derive(Clone)]
pub struct CheckinAffiliateState {
    pg: PgPool,
...
pub fn router(state: CheckinAffiliateState) -> Router {
    checkin_read_routes()
...
pub fn read_router(state: CheckinAffiliateState) -> Router {
...
async fn send_affiliate_invitation(
    ...
    // The Rust listener has no SMTP credentials. Keep the endpoint local and
    // fail closed instead of pretending delivery succeeded.
...
async fn checkin(State(state): State<CheckinAffiliateState>, headers: HeaderMap) -> Response {
    let actor = match user(&state, &headers).await {
        Ok(v) => v,
        Err(v) => return v,
    };
...
async fn aff_transfer(State(state): State<CheckinAffiliateState>, request: Request) -> Response {
...
async fn amount(State(state): State<CheckinAffiliateState>, request: Request) -> Response {
    let headers = request.headers().clone();
    let actor = match user(&state, &headers).await {
        Ok(v) => v,
        Err(v) => return v,
...
mod tests {
    use super::*;
    use crate::routes::test_support::RejectingDashboardAuth;
    use axum::{
        body::{Body, to_bytes},
        http::Request,
    };
    ...
    fn app() -> Result<Router, sqlx::Error> {
        Ok(router(CheckinAffiliateState::new(
            test_pool()?,
    ...
    fn read_app() -> Result<Router, sqlx::Error> {
        Ok(read_router(CheckinAffiliateState::new(
            test_pool()?,
            Arc::new(RejectingDashboardAuth),
        )))
    }
