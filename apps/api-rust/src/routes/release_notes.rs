...
use crate::{
    ...
    auth::{
        ...
        UserAuthPolicyError, dashboard_token_candidate, enforce_user_auth_view,
    },
...
async fn authenticated_user(
    ...
    let Some(credential) = crate::routes::legacy_http::dashboard_credential(headers) else {
        return Err(crate::routes::legacy_http::localized_dashboard_auth_error(
            headers, None,
        ));
    };
    let user = state
        ...
        .await
        .map_err(|error| {
            crate::routes::legacy_http::localized_dashboard_auth_error(headers, Some(error.kind))
        })?;
    enforce_user_auth_view(&user)
        .map_err(|error| crate::routes::legacy_http::localized_user_policy_error(headers, error))?;
...
async fn authenticated_admin(
    ...
    if user.role < ADMIN_ROLE {
        return Err(crate::routes::legacy_http::localized_user_policy_error(
            headers,
