...
/// Builds the task and status routes.
pub fn control_tasks_router(state: ControlTasksState) -> Router {
...
async fn console_access_boundary(
    State(state): State<ControlTasksState>,
    request: Request,
    next: Next,
) -> Response {
...
async fn test_status(State(state): State<ControlTasksState>, headers: HeaderMap) -> Response {
...
mod tests {
    ...
    use crate::routes::observability::ObservabilityAuthError;
    use axum::{
        body::{Body, to_bytes},
        http::Request,
    };
    ...
    fn router_for(
        ...
        control_tasks_router(ControlTasksState::new(
            store,
    ...
    -> TestResult {
        ...
        let app = control_tasks_router(ControlTasksState::new(
            store.clone(),
