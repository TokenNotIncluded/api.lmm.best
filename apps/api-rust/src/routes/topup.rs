...
pub fn router(state: TopupState) -> Router {
    topup_read_routes()
...
pub fn read_router(state: TopupState) -> Router {
...
pub fn admin_read_router(state: TopupState) -> Router {
    topup_admin_route().with_state(state)
}

fn topup_admin_route() -> Router<TopupState> {
    Router::new().route("/api/user/topup", get(all_topups))
}

fn topup_write_route() -> Router<TopupState> {
    Router::new()
        .route("/api/user/topup", post(redeem))
        .route("/api/user/topup/complete", post(complete_topup))
}

fn topup_read_routes() -> Router<TopupState> {
    Router::new()
...
async fn actor(state: &TopupState, headers: &HeaderMap) -> Result<DashboardUser, Response> {
...
async fn administrator(state: &TopupState, headers: &HeaderMap) -> Result<DashboardUser, Response> {
...
async fn forbidden_or_unauthorized(state: &TopupState, headers: &HeaderMap) -> Response {
...
async fn complete_topup(
    ...
    let response = async {
        ...
        // that predate the settlement columns; current schemas get exact
        // parity.
...
async fn topup_info(State(state): State<TopupState>, headers: HeaderMap) -> Response {
...
mod tests {
    ...
    -> TestResult {
        ...
        let state = TopupState::new(pool.clone(), Arc::new(StaticAuth { role: ROLE_ADMIN }));
