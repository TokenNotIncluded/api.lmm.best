...
use super::topup::standard_topup_settlement;
...
pub fn router(state: StripeCreemState) -> Router {
...
pub fn amount_router(state: StripeCreemState) -> Router {
...
pub fn pay_router(state: StripeCreemState) -> Router {
    Router::new()
...
fn amount_routes() -> Router<StripeCreemState> {
...
async fn actor(state: &StripeCreemState, headers: &HeaderMap) -> Result<TopupPrincipal, Response> {
...
async fn stripe_amount(State(state): State<StripeCreemState>, request: Request) -> Response {
...
async fn stripe_pay(State(state): State<StripeCreemState>, request: Request) -> Response {
    let headers = request.headers().clone();
    let actor = match actor(&state, &headers).await {
        Ok(actor) => actor,
        Err(response) => return response,
...
mod tests {
    ...
    async fn stripe_provider_failure_is_fail_closed_without_pending_order() -> TestResult {
        let store = Arc::new(Store::default());
        let app = router(StripeCreemState::new(
            store.clone(),
            Arc::new(Auth),
            Arc::new(DisabledStripeCreemGateway),
        ));
